    ...
    <section className="max-w-2xl">
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-1 border-b border-zinc-800/60 pb-6">
          <p className="text-xs uppercase tracking-[0.3em] text-zinc-400 mb-3">Pasajero</p>
        ...
        <div className="space-y-3 border-b border-zinc-800/60 pb-6">
          <p className="text-xs uppercase tracking-[0.3em] text-zinc-400">Viaje</p>
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div>
              <p className="text-zinc-400 text-xs mb-1">Origen</p>
            ...
            <div>
              <p className="text-zinc-400 text-xs mb-1">Estado solicitud</p>
              <p className="text-zinc-200">{solicitud.estado}</p>
            </div>
            <div>
              <p className="text-zinc-400 text-xs mb-1">Estado viaje</p>
              <p className="text-zinc-200">{solicitud.viaje?.estadoActual ?? "—"}</p>
            </div>
            {solicitud.precioEstimadoCents != null && (
              <div>
                <p className="text-zinc-400 text-xs mb-1">Precio</p>
                <p className="text-zinc-200">${(solicitud.precioEstimadoCents / 100).toFixed(2)}</p>
              </div>
            )}
            <div>
              <p className="text-zinc-400 text-xs mb-1">Método de pago</p>
        ...
        <div className="space-y-3">
          <p className="text-xs uppercase tracking-[0.3em] text-zinc-400">Feedback del pasajero</p>
          {tieneFeedback ? (
            <>
              ...
              {solicitud.viaje?.comentarioCalificacion ? (
                ...
                <p className="text-zinc-400 text-sm">Sin comentario.</p>
              )}
              {solicitud.viaje?.idCalificacion && (
                <p className="text-xs text-zinc-400 font-mono">ID: {solicitud.viaje.idCalificacion}</p>
              )}
            </>
          ) : (
            <p className="text-zinc-400 text-sm">El pasajero todavía no dejó feedback para este viaje.</p>
          )}
        </div>
      </div>
